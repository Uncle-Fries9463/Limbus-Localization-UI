        ...
        public static Dictionary<int, Dictionary<string, object>> Json_GetAdressBook(string Json_Файл)
        {
            ...
            for (int LineIndex = 0; LineIndex < JsonLines.Count; LineIndex++)
            {
                if (JsonLines[LineIndex].Trim().StartsWith("\"id\": "))
                {
                    ...
                    try
                    {
                        if (JsonLines[LineIndex + 5].Trim().StartsWith($"\"abilityID\": {ID}")) LineIndex_SimpleDesc1 = LineIndex + 7;
                    }
                    catch { }

                    try
                    {
                        if (JsonLines[LineIndex + 9].Trim().StartsWith($"\"abilityID\": {ID}")) LineIndex_SimpleDesc2 = LineIndex + 11;
                    }
                    catch { }

                    try
                    {
                        if (JsonLines[LineIndex + 13].Trim().StartsWith($"\"abilityID\": {ID}")) LineIndex_SimpleDesc3 = LineIndex + 15;
                    }
                    catch { }

                    try
                    {
                        if (JsonLines[LineIndex + 17].Trim().StartsWith($"\"abilityID\": {ID}")) LineIndex_SimpleDesc4 = LineIndex + 19;
                    }
                    catch { }

                    try
                    {
                        if (JsonLines[LineIndex + 21].Trim().StartsWith($"\"abilityID\": {ID}")) LineIndex_SimpleDesc5 = LineIndex + 22;
                    }
                    catch { }
